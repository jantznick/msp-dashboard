...
app.get('/admin/unauthorized-changes', isAuthenticated, isAdmin, async (req, res) => {
    try {
...
// Intake Forms
app.get('/intake/manager', async (req, res) => {
    try {
...
app.post('/intake/manager', async (req, res) => {
    ...
    try {
        ...
        if (req.session.user && req.session.user.companyId) {
            // Logged-in user with a company: Update their existing company.
            ...
            // Not logged-in, or logged-in without a company
            ...
            if (companyId === 'other' && otherCompanyName) {
                // User selected "Other" and provided a new company name
            ...
            // If a logged-in (but unassociated) user submitted changes, log it for admin review.
            if (req.session.user) {
            ...
            // If a logged-in user just created/selected their company, associate them.
            if (req.session.user && !req.session.user.companyId) {
                const updatedUser = await prisma.user.update({
                    where: { id: req.session.user.id },
                ...
                req.session.user = updatedUser; // Refresh session user data
            }
        }
        ...
        res.render('intake/manager', {
            ...
            user: req.session.user, // Use the updated user object
            companies,
...
app.get('/intake/application', async (req, res) => {
    try {
        const companies = await prisma.company.findMany({ orderBy: { name: 'asc' } });
        let defaults = null;
...
app.post('/intake/application', async (req, res) => {
    ...
    try {
        ...
        if (companyId === 'other' && otherCompanyName) {
            const newCompany = await prisma.company.upsert({
                where: { name: otherCompanyName.trim() },
                update: {}, // Don't update if it exists, just get its ID
        ...
        res.render('intake/application', { 
            ...
            companies,
            defaults: null, // Reset defaults after submission
